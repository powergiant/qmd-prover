...
test('project initialization inventories external policy, adopts, preserves, appends, and synchronizes safely', async () => {
  ...
  const canonicalBlock = must(canonicalSource.match(/<!-- qmd-prover-contract:start version=28 -->[\s\S]*?<!-- qmd-prover-contract:end -->/))[0];

  const fresh = await bareProject();
  const created = await initializeProject(fresh);
  assert.deepEqual({ ok: created.ok, status: created.status, version: created.contract_version }, { ok: true, status: 'created', version: 28 });
  ...
  const oldBlock = canonicalBlock.replace("version=28", "version=1");
...
test('dispatcher preserves JSON commands over the unified project', async () => {
  ...
  assert.equal(initialized.contract_version, 28);
  ...
  await writeFile(
    ...
    proof('thm-main-cli', 'This verifier fixture checks the proposed counterexample.', { disproof: true })
  );
...
test('skill requires a once-per-context versioned project contract preflight', async () => {
  ...
  assert.match(contract, /<!-- qmd-prover-contract:start version=28 -->/);
  ...
  assert.match(contract, /`\.disproof`/);
  assert.match(contract, /`\.abandon`/);
  assert.match(contract, /`\.draft`/);
  assert.match(contract, /There are no reserved body markers/);
  assert.match(contract, /Display-only projection of the fact's local verdict/);
  assert.match(contract, /checked in refutation mode/);
  ...
  assert.match(contract, /Global composition is deterministic\. Every fact holds exactly one of these/);
  // The eight global statuses the contract must name for an agent to act on them.
  for (const status of ['abandoned', 'broken', 'open', 'rejected', 'unverified', 'blocked', 'verified', 'disproved']) {
    assert.match(contract, new RegExp(`\\\`${status}\\\``), `contract must name the ${status} status`);
  }
  ...
  const managed = must(contract.match(/<!-- qmd-prover-contract:start version=28 -->[\s\S]*?<!-- qmd-prover-contract:end -->/))[0];
  assert.equal(must(examplePolicy.match(/<!-- qmd-prover-contract:start version=28 -->[\s\S]*?<!-- qmd-prover-contract:end -->/))[0], managed);
