 ...
 */
export const VERIFIER_TOOLS = ['file-read', 'web-search', 'code'];
function toolList(verification) {
 ...
 */
function citationRule(citations) {
    if (citations === 'strict') {
...
export function accepted(report, gapsBlock = true) {
...
export function disproved(report, gapsBlock = true) {
...
export function verificationOutcome(report, packetOrMode) {
    ...
    if (accepted(report, gapsBlock))
        return 'verified';
    if (mode === 'proof' && disproved(report, gapsBlock))
        return 'disproved';
    return 'rejected';
}
