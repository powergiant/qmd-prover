...
function optionValues(args, names, flags = new Set()) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!names.has(key) || !args[index + 1] || args[index + 1].startsWith('--'))
            throw new Error(`Invalid or missing value for ${name}`);
...
export async function main(args, { root = process.cwd(), pandoc = process.env.QMD_PROVER_PANDOC } = {}) {
    ...
    if (command === 'inspect') {
        ...
        if (subcommand === 'workspace') {
            if (tail.length !== 1)
                throw new Error('inspect workspace requires one thm-main-* ID and optional --print');
            const result = await inspectWorkspace(root, tail[0], options);
    ...
    if (command === 'dependency') {
        ...
        if (!subcommand)
            throw new Error('dependency requires an operation');
        ...
        if (retired || !operations.has(subcommand))
            throw new Error(`Unknown dependency command: ${parsed.args.join(' ')}`);
    ...
    if (command === 'check') {
        const parsed = presentation(rest);
        const [subcommand, ...tail] = parsed.args;
        if (subcommand !== 'staleness' || tail.length)
    ...
    if (command === 'submit') {
        ...
        if (subcommand !== 'proof')
            throw new Error('submit requires the proof subcommand');
        ...
        if (!proposal || (tail.length !== 1 && !(tail.length === 3 && destinationIndex === 1 && destination)))
            throw new Error('submit proof requires one proposal QMD file and optional --to QMD');
    ...
    if (command === 'workspace') {
        ...
        if (!value || tail.length)
            throw new Error('workspace requires init or inspect and one thm-main-* ID');
        if (subcommand === 'init') {
            output(await initializeWorkspace(root, value, options));
            return;
        }
    ...
    if (command === 'verification') {
        ...
        if (subcommand === 'show' && value && tail.length === 0) {
            output(await showVerification(root, value));
            return;
        }
    ...
    if (command === 'render') {
        if (rest.length)
            throw new Error('render accepts no arguments');
