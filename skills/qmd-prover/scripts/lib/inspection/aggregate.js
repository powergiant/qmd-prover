...
function compiledWorkspaceResults(index, workspace) {
...
function compiledWorkspaceManifest(index, workspace) {
    const compilation = workspace.compilation;
...
function selectedWorkspaceManifest(index, workspace, inspected) {
    if (!inspected)
        return compiledWorkspaceManifest(index, workspace);
    const projectPath = (file) => file === workspace.path || file.startsWith(`${workspace.path}/`)
        ? file
...
export function buildAggregateSnapshot(index, inspected = new Map()) {
    const workspaceManifests = new Map();
    for (const workspace of index.workspaces.filter((entry) => entry.status === 'initialized')) {
        workspaceManifests.set(workspace.id, selectedWorkspaceManifest(index, workspace, inspected.get(workspace.id) ?? workspace.snapshot ?? undefined));
    }
    const results = [];
    const workspaceByFact = new Map();
    ...
    for (const goal of index.goals)
        if (!overlayIds.has(goal.id))
            results.push(goal);
    results.sort((left, right) => left.id.localeCompare(right.id));
    const explicitOwners = new Map();
    for (const goal of index.goals)
        explicitOwners.set(goal.id, 'project-goals');
    for (const workspace of index.workspaces) {
        for (const result of workspace.compilation?.manifest.results ?? [])
            explicitOwners.set(result.id, workspace.id);
    }
    ...
    for (const result of results.filter((item) => item.origin === 'workspace')) {
        for (const dependency of result.dependencies) {
            ...
            if (owner && owner !== result.workspace)
                continue;
            edges.push({
                from: result.id,
                to: dependency,
    ...
    const adjacency = new Map(results.map((result) => [result.id, []]));
...
export function projectSourceSignature(index) {
...
export function aggregateSourceSignature(snapshot) {
    return snapshot.source_signature;
}
export async function publishAggregateSnapshot(index, snapshot, options = {}) {
    ...
    if (options.write === false || index.fatal || !compilationsComplete)
        return false;
