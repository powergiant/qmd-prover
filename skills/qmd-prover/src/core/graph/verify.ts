...
export async function verifyFacts(compilation: Compilation, context: VerificationContext, options: SelectionOptions = {}): Promise<VerificationRun> {
  ...
  const composition = (() => {
    ...
    for (const result of topologicalOrder(results)) {
      ...
      const assumptionsOf = (status: GlobalVerificationStatus): string[] => {
      ...
      const global = ((): GlobalVerification => {
        if (result.abandon) return { status: 'abandoned', blockers: [], reason: 'author-abandoned', assumptions: [] };
        ...
        if (local.status === 'rejected') return { status: 'rejected', blockers: [], reason: 'local-verification-rejected', assumptions: assumptionsOf('rejected') };
        ...
        const composed: GlobalVerificationStatus = local.status === 'not-run' ? 'verified' : local.status;
  ...
  const assumptionDiagnostics = (() => {
    if (config.verification.assumptions !== 'forbid') return [] as Diagnostic[];
    const items: Diagnostic[] = [];
    for (const result of results) {
      if (result.origin !== 'user') continue;
