...
export interface VerificationConfig {
  ...
  backend: string;
  /** Concrete model id forwarded as --model, or "" to let the backend CLI use its own default. */
  model: string;
...
function unquote(text: string): string {
  const quote = text[0];
  if (text.length >= 2 && (quote === '"' || quote === "'") && text.at(-1) === quote) return text.slice(1, -1);
  return text;
}
...
type ParsedValue = { nested: true } | { nested: false; value: JsonValue };
 ...
 */
function parseValue(rest: string, lineNo: number): ParsedValue {
  const value = rest.replace(/^[ \t]+/, '');
  if (value === '') return { nested: true };

  const first = value[0];
  if (first === '"' || first === "'") {
    const end = value.indexOf(first, 1);
    if (end === -1) throw new ConfigError(`line ${lineNo}: unterminated quoted string`);
    const after = value.slice(end + 1).replace(/^[ \t]+/, '');
    if (after !== '' && !after.startsWith('#')) throw new ConfigError(`line ${lineNo}: unexpected text after quoted value`);
  ...
  if (first === '[') {
    ...
    if (end === -1) throw new ConfigError(`line ${lineNo}: unterminated inline list`);
    const after = value.slice(end + 1).replace(/^[ \t]+/, '');
    if (after !== '' && !after.startsWith('#')) throw new ConfigError(`line ${lineNo}: unexpected text after inline list`);
    return { nested: false, value: splitList(value.slice(1, end)).map(unquote) };
  }

  let cut = -1;
  for (let i = 0; i < value.length; i += 1) {
    if (value[i] === '#' && (i === 0 || /\s/.test(value[i - 1] ?? ''))) { cut = i; break; }
  }
  const core = (cut === -1 ? value : value.slice(0, cut)).trimEnd();
  if (core === '') return { nested: true };
  if (core === 'true') return { nested: false, value: true };
  if (core === 'false') return { nested: false, value: false };
...
export function parseSimpleYaml(source: string): JsonObject {
  ...
  const stack: { indent: number; value: JsonObject }[] = [{ indent: -1, value: root }];
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (lead.includes('\t')) throw new ConfigError(`line ${lineNo}: tabs are not allowed in indentation; use spaces`);
    const match = raw.match(/^( *)([A-Za-z0-9_-]+):(.*)$/);
    if (!match) throw new ConfigError(`line ${lineNo}: expected "key: value" or "key:"`);
    const indent = match[1]?.length ?? 0;
    const key = match[2] ?? '';
    const parsed = parseValue(match[3] ?? '', lineNo);
    while ((stack.at(-1)?.indent ?? -1) >= indent) stack.pop();
    const parent = stack.at(-1)?.value;
    if (!parent) throw new ConfigError(`line ${lineNo}: inconsistent indentation`);
    if (parsed.nested) {
...
function validatedBackend(value: unknown): string {
  if (value === undefined) return defaults.verification.backend;
  if (typeof value === 'string' && VERIFIER_BACKENDS.includes(value)) return value;
...
function validatedModel(value: unknown): string {
  if (value === 'configurable') throw new ConfigError('verification.model: "configurable" is no longer a value; use "" for the CLI\'s default model');
...
export async function loadConfig(root: string): Promise<QmdProverConfig> {
  ...
  try {
    ...
    if (error instanceof ConfigError) throw new ConfigError(`.qmd-prover/config.yml ${error.message}`);
    throw error;
  }
}
