...
import {
  ABANDON_CLASS, ASSUMED_CLASS, DISPROOF_CLASS, DRAFT_CLASS, KIND_BY_PREFIX, RESULT_KINDS, SCHEMA_VERSION,
...
export function factIntent(result: SemanticResult): FactIntent {
  ...
  if (result.assumed) return 'assumed';
...
export function preVerificationStatus(result: SemanticResult, broken: boolean): GlobalVerificationStatus {
  ...
  if (result.intent === 'assumed') return 'verified';
        ...
        for (const entry of entries) {
          ...
          if (entry.type === 'proof') {
            ...
            const parsedProof = (() => {
              ...
              if (assumed && draft) diagnostics.push(diagnostic('error', 'ASSUMED_DRAFT', `Proof of @${target || '?'} carries both .assumed and .draft; a fact is taken as given or is unfinished, not both`, file.relative, line, target));
              if (assumed && disproof) diagnostics.push(diagnostic('error', 'ASSUMED_DISPROOF', `Proof of @${target || '?'} carries both .assumed and .disproof; assert an assumed result instead of an unargued refutation`, file.relative, line, target));
              ...
              if (!abandon && !draft && !assumed && !present) diagnostics.push(diagnostic('warning', 'PROOF_EMPTY', `Proof of @${target || '?'} is empty; @${target || '?'} is open`, file.relative, line, target));
          ...
          const parsedResult = (() => {
            ...
            if (!present) diagnostics.push(diagnostic('error', 'STATEMENT_MISSING', `${id} requires a nonempty statement body`, file.relative, line, id));
            ...
            if (disproof) diagnostics.push(diagnostic('error', 'RESULT_DISPROOF_FORBIDDEN', `${id} must not carry .disproof on its result body; put .disproof on its linked .proof block`, file.relative, line, id));
            // A result body taken as given may not also declare itself unfinished: the two are opposite claims.
            if (assumed && draft) diagnostics.push(diagnostic('error', 'ASSUMED_DRAFT', `${id} carries both .assumed and .draft; a fact is taken as given or is unfinished, not both`, file.relative, line, id));
  ...
  const linked = (() => {
    ...
    const overlay = (() => {
      ...
      for (const [target, allProofs] of proofsByTarget) {
        ...
        if (!result) {
          for (const proof of allProofs) diagnostics.push(diagnostic('error', 'PROOF_TARGET_UNKNOWN', `Proof target @${target} does not exist`, proof.file, proof.line, target));
          continue;
        }
        ...
        if (result.kind === 'definition' && proof.refutation) diagnostics.push(diagnostic('error', 'DEFINITION_DISPROOF_FORBIDDEN', `@${target} is a definition and cannot carry a .disproof proof; refute a theorem-like claim about it instead`, proof.file, proof.line, target));
