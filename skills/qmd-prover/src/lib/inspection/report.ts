...
import type {
  AiCheck, DependencyGraph, Diagnostic, GraphFindings, GraphNode, InspectionVerificationSummary,
...
interface CheckedReportItem {
  id: string;
  programmatic: ProgrammaticFactCheck;
  ai: AiCheck;
}
...
export function printReport(input: OperationResult): string {
  ...
  if (result.staleness) {
    ...
    for (const item of result.staleness.changed ?? []) {
      ...
      if (item.previous !== undefined) lines.push(`    previous: ${JSON.stringify(item.previous)}`);
  ...
  if (checks.length) {
    ...
    for (const check of [...checks].sort((left, right) => left.id.localeCompare(right.id))) {
      ...
      if (check.ai.reason) lines.push(`    AI: ${check.ai.reason}`);
      if (check.ai.error) lines.push(`    AI error${check.ai.code ? ` (${check.ai.code})` : ''}: ${check.ai.error}`);
      if (check.ai.details?.command) lines.push(`    verifier command: ${check.ai.details.command}`);
      if (check.ai.details?.exit_code !== undefined || check.ai.details?.signal) lines.push(`    verifier termination: exit=${check.ai.details.exit_code ?? 'none'}, signal=${check.ai.details.signal ?? 'none'}`);
      if (check.ai.details?.stderr_excerpt) lines.push(`    verifier stderr: ${check.ai.details.stderr_excerpt}`);
      if (check.ai.details?.stdout_excerpt) lines.push(`    verifier stdout: ${check.ai.details.stdout_excerpt}`);
      if (check.ai.remediation) lines.push(`    remediation: ${check.ai.remediation}`);
      if (check.ai.report) {
        if (check.ai.report.summary) lines.push(`    summary: ${check.ai.report.summary}`);
        for (const error of check.ai.report.critical_errors ?? []) lines.push(`    critical error: ${error}`);
        for (const gap of check.ai.report.gaps ?? []) lines.push(`    gap: ${gap}`);
        for (const comment of check.ai.report.nonblocking_comments ?? []) lines.push(`    comment: ${comment}`);
        if (check.ai.report.repair_hints) lines.push(`    repair hints: ${check.ai.report.repair_hints}`);
      }
    }
  }
