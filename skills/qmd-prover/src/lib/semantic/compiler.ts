...
import { accepted, checkerContract } from '../verification/protocol.js';
import {
  KIND_BY_PREFIX, RESULT_KINDS, SEMANTIC_ID_PATTERN, SEMANTIC_PREFIX_PATTERN, isControlMarker
} from '../shared/core.js';
...
async function initializeAux(root: string): Promise<void> {
...
export async function compileProject(root = process.cwd(), options: CompilerOptions = {}): Promise<Compilation> {
    ...
    try {
      ...
      if (semanticMode === 'project-goals') {
        for (const entry of parsedEntries.filter((item) => item.type === 'proof')) {
          ...
          if (!target.startsWith(config.goals['id-prefix'])) continue;
          const content = proofContent(entry.blocks);
          if (content.marker === 'VERIFIED' || content.marker === 'REVOKED') diagnostics.push(diagnostic(
            'warning', 'LEGACY_CANONICAL_VERIFICATION',
            `Linked proof of @${target} contains the legacy ${content.marker} marker; workspace inspection ignores it and never rewrites user notes`,
            file.relative, locateProof(source, target)?.startLine, target
          ));
        }
      }
  ...
  const fileMap = new Map<string, SourceFileRecord>(files.map((file) => [file.path, file]));
  const importAdjacency = new Map<string, string[]>(files.map((file) => [file.path, []]));
  ...
  const dependencyAdjacency = new Map<string, string[]>(allResults.map((result) => [result.id, result.dependencies.filter((id) => byId.has(id))]));
  ...
  const verification = semanticMode !== 'full'
    ? {}
    : await readJson<Record<string, UnknownRecord>>(path.join(root, AUX, 'verification', 'index.json'), {});
  const evidence = semanticMode !== 'full' ? new Map<string, VerificationEvidence>() : await verificationEvidence(root, verification);
  ...
  for (const result of allResults) {
    if (semanticMode === 'project-goals') {
      result.status = 'open';
      if (result.marker && result.marker !== 'OPEN') diagnostics.push(diagnostic(
        'warning', 'LEGACY_CANONICAL_VERIFICATION',
        `${result.id} contains the legacy ${result.marker} marker; workspace inspection ignores it and never rewrites user notes`,
        result.file, result.proof_line ?? result.line, result.id
      ));
    } else result.status = verificationStatus(result, verification, evidence);
  }
  ...
  const manifest: Manifest = { schema_version: 2, files, results: allResults, proofs: allProofs.map(({ blocks, markers, marker_index, ...proof }) => proof) };
